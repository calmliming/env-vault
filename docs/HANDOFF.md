...
- **一次纳管多个项目**（选一个装着十几个仓库的父目录）。现在做不到：
  `scanProject` 只认一个项目根，把 workspace 根丢进去会把所有仓库的 `.env`
  拍平成同一个项目，Git 风险检查还会定位到错的仓库根。扫描层和数据模型的活。
...
- **把 `envvault` 装进 PATH** 是发布时的事（electron-builder 的安装器）。
  现在打包后是 `EnvVault.exe run -- <命令>`。

## 8. 来自踩坑的经验

**一条从来没红过的断言，要么是代码真的对，要么是断言根本够不着它 ——
这两种情况在测试报告上长得一模一样。** 这条在这个仓库里已经踩中七次：
