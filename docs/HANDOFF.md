...
pnpm verify    # 全套：201 单元测试 + 200 核心断言 + 121 界面断言，约 2 分钟
               # 单元测试是 200 通过 + 1 跳过：write.test.ts 的 POSIX 权限位，
...
**九处操作会改变监听集合**（导入 / 移除 / 重扫 / 以磁盘为准 / 以记录为准 /
编辑变量 / 删除变量 / 凭据同步 / 生成 `.env.example`），每一处都要调
`refreshWatchTargets()`。漏掉的后果是静默的：监听器还活着，但拿旧哈希去比新文件。
...
## 7. 下一步：阶段 5c，加密导出与导入导出包

MVP 清单（计划 §11）到 4b 就全成立了，5a 补上 CLI 注入，5b 补上模板生成。剩下的：

- **加密导出，以及导入导出包。** 计划 §7：导出要二次确认、优先导出加密包。

  ⚠️ **规格 §9 里那个词是「批量导入」，它指的就是「导入加密导出包」** ——
  导出的镜像操作。这个词从阶段 0 起一路被抄进各版文档却从没定义过，
  5b 收尾时才澄清（计划 §12 第 10 条记了完整来龙去脉）。
  它**不是**「一次纳管多个项目」，也**不是**「从别的工具的格式导入」；
  而「扫一个目录、勾一批文件入库」那个意义上的批量导入，阶段 1 就交付了。

  **前置结论已经定了：加密导出用「用户口令 + KDF」，不是系统密钥库派生。**
  密钥库派生出来的包只有本机能解，而「换机器 / 备份」正是导出的用途 ——
  一个打不开的备份不叫备份。代价是引入一条新的用户口令处理路径，
  而口令强度不由我们控制，这一点要在界面上直说。
- **1Password / Bitwarden / Doppler 适配。** 它们需要机器上装有各自的 CLI，
  验收里只能全程用假的 —— 而一个只验过假实现的集成等于没验。
  真要做的话，先想清楚怎么验，再想怎么写。

四条外部边界都已经开了（出站流量、执行外部程序、系统剪贴板、把 Key 交给
子进程），5b 又开了第五条**产物**边界（写一个要进 Git 的文件）。
它们各自只覆盖一条路。**导出是下一条，不要默认沿用前面的结论。**

导出要回答的问题和 5a「不落盘」是同一个形状：**这次操作让磁盘上多出了什么。**
用前后快照答，别退回白名单排除（§8）。
...
这两种情况在测试报告上长得一模一样。** 这条在这个仓库里已经踩中五次：
